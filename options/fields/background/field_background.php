	...
	*/
	function render(){
		...
		$css_options = array(
			'repeat'	 => array(
				''		  => __('Default', 'crypto' ),
				'no-repeat' => __('No Repeat', 'crypto' ),
				'repeat'	=> __('Repeat All', 'crypto' ),
				'repeat-x'  => __('Repeat Horizontally', 'crypto' ),
				'repeat-y'  => __('Repeat Vertically', 'crypto' ),
				//'inherit'   => 'Inherit',
			),
			'attachment' => array(
				''		=> __('Default', 'crypto' ),
				'fixed'   => __('Fixed', 'crypto' ),
				'scroll'  => __('Scroll', 'crypto' ),
				//'inherit' => 'Inherit',
			),
			'position'   => array(
				''			  => __('Default', 'crypto' ),
				'left top'	  => __('Left Top', 'crypto' ),
				'left center'   => __('Left center', 'crypto' ),
				'left bottom'   => __('Left Bottom', 'crypto' ),
				'center top'	=> __('Center Top', 'crypto' ),
				'center center' => __('Center Center', 'crypto' ),
				'center bottom' => __('Center Bottom', 'crypto' ),
				'right top'	 => __('Right Top', 'crypto' ),
				'right center'  => __('Right center', 'crypto' ),
				'right bottom'  => __('Right Bottom', 'crypto' ),
			),
			'size'	   => array(
				''		=> __('Default', 'crypto' ),
				//'inherit' => 'Inherit',
				'cover'   => __('Cover', 'crypto' ),
				'contain' => __('Contain', 'crypto' ),
			),
			'parallax'   => array(
				'0' => __('Off', 'crypto' ),
				'1' => __('On', 'crypto' ),
		...
		if ( $this->field['options']['color'] !== false ) {
			...
			echo '<div class="bg-opt-input-label">'.__( 'Background Color:', 'crypto' ).'</div>';
		...
		// Tabs
		echo '<div class="bg-opt-input-label">'.__( 'Background Image:', 'crypto' ).'</div>';
		echo '<fieldset class="green buttonset buttonset-tabs ui-buttonset">';
			if ( $this->field['options']['image_pattern'] !== false ) {
				...
				echo '<label id="nhp-opts-button" for="'.$this->field['id'].'_pattern" class="buttonset-tab ui-button ui-state-default ui-button-text-only"><span class="ui-button-text">'.__('Pattern', 'crypto' ).'</span></label>';
			}
			if ( $this->field['options']['image_upload'] !== false ) {
				...
				echo '<label id="nhp-opts-button" for="'.$this->field['id'].'_upload" class="buttonset-tab ui-button ui-state-default ui-button-text-only"><span class="ui-button-text">'.__('Upload', 'crypto' ).'</span></label>';
			}
			if ( $this->field['options']['gradient'] !== false ) {
				...
				echo '<label id="nhp-opts-button" for="'.$this->field['id'].'_gradient" class="buttonset-tab ui-button ui-state-default ui-button-text-only"><span class="ui-button-text">'.__('Gradient', 'crypto' ).'</span></label>';
			}
		...
		if ( $this->field['options']['image_upload'] !== false ) {
			...
			echo ' <a href="javascript:void(0);" class="nhp-opts-upload button-secondary"'.$upload.' rel-id="'.$this->field['id'].'_image_upload">'.__('Browse', 'crypto' ).'</a>';
			echo ' <a href="javascript:void(0);" class="nhp-opts-upload-remove"'.$remove.' rel-id="'.$this->field['id'].'_image_upload">'.__('Remove Upload', 'crypto' ).'</a>';
			echo '</fieldset>';
			...
			if ( $this->field['options']['repeat'] !== false ) {
				...
				echo '<label for="'.$this->field['id'].'_repeat" class="bg-opt-input-label">'.__( 'Background Repeat:', 'crypto' ).'</label>';
			...
			if ( $this->field['options']['attachment'] !== false ) {
				...
				echo '<div class="bg-upload-select">';
				echo '<label for="'.$this->field['id'].'_attachment" class="bg-opt-input-label">'.__( 'Background Attachment:', 'crypto' ).'</label>';
			...
			if ( $this->field['options']['position'] !== false ) {
				...
				echo '<div class="bg-upload-select">';
				echo '<label for="'.$this->field['id'].'_position" class="bg-opt-input-label">'.__( 'Background Position:', 'crypto' ).'</label>';
			...
			if ( $this->field['options']['size'] !== false ) {
				...
				echo '<div class="bg-upload-select">';
				echo '<label for="'.$this->field['id'].'_size" class="bg-opt-input-label">'.__( 'Background Size:', 'crypto' ).'</label>';
		...
		if ( $this->field['options']['gradient'] !== false ) {
			...
			echo '<div class="bg-opt-input-label">' . __('From:', 'crypto' ) . '</div><input type="text" id="'.$this->field['id'].'_gradient_from" name="'.$this->args['opt_name'].'['.$this->field['id'].'][gradient][from]" value="'.$this->value['gradient']['from'].'" class="popup-colorpicker" data-default-color="'.$defaults['gradient']['from'].'" style="width:70px;"/>';
			echo '</div>';
			echo '<div class="color-gradient-step-wrapper">';
			echo '<div class="bg-opt-input-label">' . __('To:', 'crypto' ) . '</div><input type="text" id="'.$this->field['id'].'_gradient_to" name="'.$this->args['opt_name'].'['.$this->field['id'].'][gradient][to]" value="'.$this->value['gradient']['to'].'" class="popup-colorpicker" data-default-color="'.$defaults['gradient']['to'].'" style="width:70px;"/>';
			echo '</div>';

			echo '<div class="color-gradient-direction-wrapper">';
			echo '<label for="'.$this->field['id'].'_gradient_direction" class="bg-opt-input-label">' . __('Direction:', 'crypto' ) . '</label>';
			echo '<select id="'.$this->field['id'].'_gradient_direction" name="'.$this->args['opt_name'].'['.$this->field['id'].'][gradient][direction]" class="color-gradient-direction">';
				echo '<option value="horizontal"' . selected( $this->value['gradient']['direction'], 'horizontal', false ) . '>' . __( 'Horizontal', 'crypto' ) . '</option>';
				echo '<option value="vertical"' . selected( $this->value['gradient']['direction'], 'vertical', false ) . '>' . __( 'Vertical', 'crypto' ) . '</option>';
			echo '</select>';
			echo '</div>';
			echo '</div>';
			echo '</div>';
		...
		if ( $this->field['options']['parallax'] !== false ) {
			...
			echo '<div class="bg-opt-input-label">'.__( 'Parallax Effect:', 'crypto' ).'</div>';
