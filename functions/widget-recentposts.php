	...
	class mts_recent_posts_widget extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				...
				sprintf( __('%sRecent Posts', 'crypto' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ),
				array( 'description' => __( 'Display the most recent posts from all categories', 'crypto' ) )
			);
		}
	 	...
	 	public function form( $instance ) {
			...
			$title = isset( $instance[ 'title' ] ) ? $instance[ 'title' ] : __( 'Recent Posts', 'crypto' );
			...
			$show_thumb2 = isset( $instance[ 'show_thumb2' ] ) ? esc_attr( $instance[ 'show_thumb2' ] ) : 0;
