...
class mtsGoogleTypography {
	...
	function help_tab() {
		...
		$adding_title				= __('Adding A Collection', 'crypto' );
		$adding_content				= '<p>'.__('To add a new font for use on your site. Click the "Add New" button on the top left of the page near the "Google Typography" title.', 'crypto' ).'</p>';
		$adding_content				.= '<p>'.__('Once added, a new font row will appear on the page below. Next you can continue to customize your font (more info in the "Customizing" help tab).', 'crypto' ).'</p>';
		//$adding_content			 .= '<p><a href="https://vimeo.com/67957799" target="_blank">'.__('Watch The Video Tutorial &rarr;', 'crypto' ).'</a></p>';
		$customizing_title 		= __('Customizing', 'crypto' );
		$customizing_content	= '<p>'.__('Customizing fonts is easy; after adding a new font row you can then customize the following font attributes:', 'crypto' ).'</p>';
		$customizing_content .= '<ul><li><b>'.__('Preview Text', 'crypto' ).'</b> - '.__('Used for live previewing your changes. This text does not appear anywhere on your website.', 'crypto' ).'</li><li><b>Preview Background Color</b> - Allows you to swap between light and dark backgrounds when previewing this font.</li><li><b>'.__('Font Family', 'crypto' ).'</b> - '.__('The font family to use for this font. Choose from a real-time list of all available Google Fonts.', 'crypto' ).'</li><li><b>'.__('Font Variant', 'crypto' ).'</b> - '.__('The variant to use for this font. Note: Each font has it\'s own variant options.', 'crypto' ).'</li><li><b>'.__('Font Size', 'crypto' ).'</b> - '.__('The size you would like this font to be.', 'crypto' ).'</li><li><b>'.__('Font Color', 'crypto' ).'</b> - '.__('The color you\'d like to use for this font.', 'crypto' ).'</li><li><b>'.__('CSS Selectors', 'crypto' ).'</b> - '.__('The HTML tags or CSS selectors you\'d like this font to apply to (more info in the "CSS Selectors" help tab). You can specify multiple selectors separated by comma\'s. Ex: h1, #some_id, .some_class', 'crypto' ).'</li></ul>';
		$selectors_title 			= __('CSS Selectors', 'crypto' );
		$selectors_content		= '<p>' . __('CSS Selectors are used to hook your font rows into your actual website. Once you\'ve added, customized, and defined CSS selectors for your fonts, Google Typography will automatically insert all the necessary CSS into your website.', 'crypto' ) . '</p>';
		$selectors_content		= '<p>' . __('Here are some examples of the selectors you can use:', 'crypto' ) . '</p>';
		$selectors_content	 .= '<ul><li><b>'.__('IDs', 'crypto' ).':</b> '.__('#selector', 'crypto' ).'</li><li><b>'.__('Classes:', 'crypto' ).':</b> '.__('.selector', 'crypto' ).'</li><li><b>'.__('HTML Tags', 'crypto' ).':</b> '.__('span', 'crypto' ).'</ul>';
		$selectors_content	 .= '<p><b>'.__('Example', 'crypto' ).':</b> '.__('#selector span.date', 'crypto' ).'</p>';
	...
	function plugin_link($links, $file) {
		if ( $file == 'google-typography/google-typography.php' ) {
			$links['settings'] = sprintf( '<a href="%s"> %s </a>', admin_url( 'themes.php?page=typography' ), __( 'Settings', 'crypto' ) );
		}
		return $links;
	}
	...
	function options_ui() {
		...
		$title						= __('Theme Typography', 'crypto' );
		$loading					= __('Loading Your Collections', 'crypto' );
		$add_new					= __('Add New Collection', 'crypto' );
		$reset						= __('Reset Collections', 'crypto' );
		$preview_text   			= __('Type in some text to preview...', 'crypto' );
		$collection_title 			= __('New Collection', 'crypto' );
		//$this->preview_text = $preview_text; // to pass it to JS
		$preview_hint   			= __('Preview Background Color', 'crypto' );
		//$font_family_title  	= __('Font family...', 'crypto' );
		$std_font_family_title  	= __('Standard Fonts', 'crypto' );
		$google_font_family_title  	= __('Google Fonts', 'crypto' );
		$font_variant_title 		= __('Variant...', 'crypto' );
		$font_size_title 			= __('Size...', 'crypto' );
		$backup_font_title			= __('Fallback:', 'crypto' );
		$css_selectors_title		= __('CSS Selectors:', 'crypto' );
		$additional_css_title		= __('Additional CSS:', 'crypto' );
		$delete_button_text			= __('Delete', 'crypto' );
		$save_button_text			= __('Save', 'crypto' );
		
		$welcome_title 				= __('Theme Typography', 'crypto' );
		$welcome_subtitle			= __('Get started in 3 steps.', 'crypto' );//.'<a href="https://vimeo.com/67957799" target="_blank">'.__('Watch the video tutorial &#x2192;', 'crypto' ).'</a>';
		$step_1_title 				= __('1. Pick A Font', 'crypto' );
		$step_1_desc				= __('Choose from any of the 600+ Google Fonts.', 'crypto' );
		$step_2_title 				= __('2. Customize It', 'crypto' );
		$step_2_desc				= __('Pick a size, variant, color and more.', 'crypto' );
		$step_3_title 				= __('3. Attach It', 'crypto' );
		$step_3_desc				= __('Attach your font to any CSS selector(s).', 'crypto' );
		$year						= date("Y");
		$themename = MTS_THEME_NAME;
		
		$character_sets_title = __('Character sets', 'crypto' );
		$character_sets_desc = __('Choose the character sets you wish to include. Please note that not all sets are available for all fonts.', 'crypto' );
		...
		$character_sets = array(
			'latin' 		=> __('Latin', 'crypto' ),
			'latin-ext' 	=> __('Latin Extended', 'crypto' ),
			'cyrillic' 		=> __('Cyrillic', 'crypto' ),
			'cyrillic-ext'	=> __('Cyrillic Extended', 'crypto' ),
			'greek'			=> __('Greek', 'crypto' ),
			'greek-ext'		=> __('Greek Extended', 'crypto' ),
			'vietnamese'	=> __('Vietnamese', 'crypto' ),
			'khmer'			=> __('Khmer', 'crypto' ),
			'devanagari'	=> __('Devanagari', 'crypto' ),
		);
...
EOT;
	...
	public function generate_image($font) {
		...
		if ( !$ttf_path || !file_exists( $ttf_path ) ) {
			error_log( 'BYMe/Typography: Could not load $ttf_path: ' . $ttf_path );
			return;
		}
	...
	public function save_image( $image, $font ) {
		...
		if ( !is_dir( $dir ) && !wp_mkdir_p( $dir ) ) { 
			error_log( "BYMe/Typography: Please check permissions. Could not create directory $dir" );
			return;
		}
		...
		if ( !$image_file ) {
			error_log( "BYMe/Typography: Please check permissions. Could not write image to $dir" );
			return;
		}
	}
	...
	public function get_remote_ttf($font) {
		...
		// Check file saved
		if ( !$ttf_file_path ) {
			error_log( "BYMe/Typography: Please check permissions. Could not write font to $dir." );
			return false;
		}
	...
	public function get_remote_ttf_contents($ttf_url) {
		...
		if ( empty( $ttf_url ) ) {
			error_log( 'BYMe/Typography: Font URL not set.' );
			return false;
		}
		...
		if ( is_a( $response, 'WP_Error') ) {
			error_log( "BYMe/Typography: Attempt to get remote font returned an error.<br/>$ttf_url" );
			return false;
		}
	...
	function admin_scripts() {
		...
		wp_localize_script('google-typography', 'googletypography', array(
				'reset_confirm' => __("Are you sure you want to revert to the theme's default collections? Note: You will lose any custom collections you've created.", 'crypto' ),
				'delete_confirm' => __("Are you sure you want to delete this collection?", 'crypto' )
			));
