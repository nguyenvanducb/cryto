...
class MTS_Options_Tab_Support {
	...
	public function add_support_tab( $sections ) {
		...
		$sections['support'] = array(
			...
			sprintf( __( 'We offer technical support through our %s. Please <strong>copy and paste the following information in your ticket</strong> when contacting support:' , 'crypto' ), '<a href="https://community.BYMe.com/forum/7-theme-support/" target="_blank">' . __( 'Support Ticket System', 'crypto' ) . '</a>' )
			 . '</p>',
	...
	public function add_support_tab_content() {
		...
		<div class="mts-options-extra-tab-content">
			...
			<textarea class="large-text" id="mts-debug-data-field" rows="16" readonly="readonly" disabled><?php _e( 'Generating log. Please wait...', 'crypto' ); ?></textarea>
	...
	public function support_buttons() {
		...
		<div class="mts-help-buttons">
			<button type="button" class="button mts-support-copy" disabled>
				...
				<?php _e( 'Copy Data for Support Request', 'crypto' ); ?>
			</button> 
			<a href="https://community.BYMe.com/forum/7-theme-support/" target="_blank" class="button button-primary mts-support-link">
				<!-- <span class="dashicons dashicons-external"></span> -->
				<?php _e( 'Open Support Forum', 'crypto' ); ?>
			</a>
		</div>
		<?php
	}
	...
	public function get_debug_data() {
		...
		$data['environment'] = array(
			'title' => __( 'Environment', 'crypto' ),
			'data' => $this->get_environment_info(),
		);

		$data['wp'] = array(
			'title' => __( 'WordPress', 'crypto' ),
			'data' => $this->get_wp_info(),
		);

		$data['plugins'] = array(
			'title' => __( 'Plugins', 'crypto' ),
			'data' => $this->get_plugins_info(),
		);

		$data['themes'] = array(
			'title' => __( 'Theme', 'crypto' ),
			'data' => $this->get_theme_info(),
		);

		$data['settings'] = array(
			'title' => __( 'Theme Settings', 'crypto' ),
